...
const MattermostWebhook = ({ projectId }) => {
  const { token } = useAuth();
  ...
  // Load existing webhook settings (same as Discord, but we'll check for Mattermost URL)
  useEffect(() => {
    const fetchSettings = async () => {
      ...
      try {
        ...
        if (response.data.status) {
          ...
          if (notifications) {
            // Only set if it's a Mattermost webhook
            if (notifications.webhook && 
                (notifications.webhook.includes('hooks.mattermost.com') || 
                 notifications.webhook.includes('/hooks/'))) {
  ...
  return (
    <Box sx={{ mt: 2, p: 3, border: '1px solid #e0e0e0', borderRadius: 1 }}>
      ...
      <Typography variant="body2" color="text.secondary" paragraph>
        Enter your Mattermost webhook URL to receive notifications about task updates and comments.
      </Typography>
      ...
      {testStatus === 'success' && (
        <Alert severity="info" sx={{ mb: 2 }}>
          Test notification sent successfully! Check your Mattermost channel.
        </Alert>
      )}
      ...
      <TextField
        ...
        placeholder="https://your-mattermost-instance.com/hooks/..."
        margin="normal"
        helperText="Create a webhook URL in your Mattermost integration settings"
      />
...
export default MattermostWebhook;